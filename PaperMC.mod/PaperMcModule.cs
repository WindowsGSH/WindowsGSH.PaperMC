    ...
    IModuleReadinessCapability
{
    ...
    public Task<IReadOnlyList<ReadinessCheckResult>> CheckReadinessAsync(ServerInstance instance, CancellationToken cancellationToken)
    {
        ...
        var jarPath = TryResolveServerJarPath(instance.InstallPath, instance.Settings);
        checks.Add(jarPath != null
            ? ReadinessCheckResult.Pass("PaperMC server jar", $"Found: {jarPath}")
            : ReadinessCheckResult.Fail("PaperMC server jar", "No configured or recognized PaperMC server jar was found. Run Update to download it."));

        var java = _javaRuntimeLocator.Locate(instance.AppSettings.Java.RuntimePath);
        checks.Add(java.Found
            ? ReadinessCheckResult.Pass("Java runtime", $"Found Java {java.MajorVersion?.ToString() ?? "unknown version"}: {java.ExecutablePath}")
            : ReadinessCheckResult.Fail("Java runtime", java.ErrorMessage ?? "A working java.exe was not found."));

        checks.Add(ReadBool(instance, "eula.accepted", defaultValue: false)
            ? ReadinessCheckResult.Pass("Minecraft EULA", "The server configuration records EULA acceptance.")
            : ReadinessCheckResult.Fail("Minecraft EULA", "Accept the Minecraft EULA in the server configuration before starting."));
